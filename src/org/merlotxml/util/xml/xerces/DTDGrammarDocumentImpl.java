...
public class DTDGrammarDocumentImpl implements GrammarDocument {
    ...
    public DTDGrammarDocumentImpl(Grammar[] grammars) {
        ...
        while (index != -1) {
            ...
            if (found) {
                DTDGrammarComplexTypeImpl complexType =
                    new DTDGrammarComplexTypeImpl(this, index, elementDecl);
        ...
        _complexTypes = (DTDGrammarComplexTypeImpl[])complexTypes.toArray(_complexTypes);
        //analyzeContentSpecs();
    }

    public GrammarComplexType getGrammarComplexType(GrammarComplexType parent, String name) {
        // Can ignore parent for DTDs.
    ...
    protected void analyzeContentSpecs() {
        ...
        while (found) {
            ...
            if (spec.value != null) {
                ...
                if (spec.value instanceof CMNode)
                    MerlotDebug.msg(class1 + " is a CMNode");
            }
            String class2 = "";
            if (spec.otherValue != null) {
                class2 = spec.otherValue.getClass().getName();
                if (spec.otherValue instanceof CMNode)
                    MerlotDebug.msg(class2 + " is a CMNode");
            }
            MerlotDebug.msg("XMLContentSpec: type=" + spec.type + " value="
                 + class1 + "(" + spec.value + ") otherValue="
                 + class2 + "(" + spec.otherValue + ")");
            index++;
