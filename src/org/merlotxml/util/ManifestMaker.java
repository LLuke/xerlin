...
// Copyright 2000 ChannelPoint, Inc., All Rights Reserved.
...
public class ManifestMaker 
{
	...
	out.println("  Options:");
	/*	out.println("           -t <filename>     Read in a template manifest file, replacing those");
	out.println("                             fields that are set on the command line");
	*/
	...
	//	out.println("           --sealed=true     Seal jarfile");
	...
	try {
		...
		if (arg.startsWith("-")) {
		    ...
		    // must be last arg and must be filename
		    if (i+1 < args.length) {
    ...
    public static String getNextArg(String[] args, int i) 
	throws Exception
    {
	if (i+1 < args.length) {
	    String arg  = args[i];
	...
	try {
	    ...
	    // setup the implementation versoin (buildID)
